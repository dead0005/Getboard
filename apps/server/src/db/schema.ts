...
export const striverQuestions = sqliteTable('striver_questions', {
...
export const leetcodeQuestions = sqliteTable('leetcode_questions', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  titleSlug: text('title_slug').notNull().unique(),
  title: text('title').notNull(),
});

export const striverQuestionMappings = sqliteTable('striver_question_mappings', {
...
export const solveRecords = sqliteTable('solve_records', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  leetcodeQuestionId: integer('leetcode_question_id').notNull().references(() => leetcodeQuestions.id, { onDelete: 'cascade' }),
