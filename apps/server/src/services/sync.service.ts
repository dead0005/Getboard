  ...
  async runSync() {
    ...
    try {
      ...
      if (solvedSlugs.length > 0) {
        const lcQuestions = await db.select()
          .from(schema.leetcodeQuestions)
          .where(inArray(schema.leetcodeQuestions.titleSlug, solvedSlugs));
          
        for (const q of lcQuestions) {
          const existing = await db.select()
            .from(schema.solveRecords)
            .where(eq(schema.solveRecords.leetcodeQuestionId, q.id));
      ...
      await db.update(schema.syncRuns)
        .set({ status: 'success', problemsAdded: newSolvesCount })
