    ...
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="space-y-4">
        {topics?.map((topic: any) => {
          ...
          const solvedCount = topic.questions.filter((q: any) => q.isSolved).length;
                  ...
                  {topic.questions.map((q: any) => (
                    <div key={q.id} className="p-4 px-6 flex items-center justify-between hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex items-center gap-2">
                        {q.striverLink && (
                          <a href={q.striverLink} target="_blank" rel="noreferrer" className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Article/Video">
                            <ExternalLink className="w-4 h-4" />
                          </a>
                        )}
                        {q.leetcodeSlug && (
                          <a href={`https://leetcode.com/problems/${q.leetcodeSlug}`} target="_blank" rel="noreferrer" className="px-3 py-1.5 text-sm font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
                            LeetCode
                          </a>
                        )}
                      </div>
